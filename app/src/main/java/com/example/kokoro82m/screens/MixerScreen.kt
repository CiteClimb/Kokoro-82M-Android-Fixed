...
fun MixerScreen(
    ...
    var selectedStyles by remember {
        mutableStateOf(listOf("am_adam", "am_michael", "af_nicole"))
    }
    var weights by remember {
        mutableStateOf(mapOf("am_adam" to 0.5f, "am_michael" to 0.25f, "af_nicole" to 0.5f))
    }
    var interpolationMode by remember {
        mutableStateOf(InterpolationMode.LINEAR)
    }

    var text by remember { mutableStateOf("Generic placeholder text.") }
    ...
    Column(
        ...
        Row(
            ...
            Button(
                onClick = {
                    selectedStyles = listOf("af_adam", "am_michael", "af_nicole")
                    weights = mapOf("am_adam" to 0.5f, "am_michael" to 0.25f, "af_nicole" to 0.5f)
                }
            ) {
                Text("Reset")
            }
            ...
            Button(
                ...
                Text(if (isProcessing) "GPU Processing..." else "Apply Mix and Play")
            }
        }
    }
}
...
private suspend fun mixStyles(
    ...
    require(styles.isNotEmpty()) { "There must be at least one style selected" }
...
private fun StyleSelector(
    ...
    Column {
        Text("Selected Voice Styles:", style = MaterialTheme.typography.labelLarge)
...
enum class InterpolationMode(val displayName: String) {
    LINEAR("Linear (Faster)"),
    SPHERICAL("Spherical (Realistic) [BROKEN]")
}
